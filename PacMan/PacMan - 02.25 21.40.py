w = 600 #Размер окна
h = 30 #Расстояние от края до поля
r = 12 #Радиус PACMANа
...
def draw_Blinky ():
    ...
    if vxB == 0 and vyB == 0:
        draw_stop (xB, yB)
        canv.create_rectangle (xB - 8, yB - 6, xB - 4, yB - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
        canv.create_rectangle (xB + 4, yB - 6, xB + 8, yB - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
    else:
        if x == xB:
            draw_stop (xB, yB)
            if y > yB:
                canv.create_rectangle (xB - 8, yB - 4, xB - 4, yB, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xB + 4, yB - 4, xB + 8, yB, fill = 'blue', outline = 'blue', tag = 'Ghost')
            else:
                canv.create_rectangle (xB - 8, yB - 10, xB - 4, yB - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xB + 4, yB - 10, xB + 8, yB - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
        elif x > xB:
            draw_right (xB, yB)
            if y >= yB:
...
def draw_Pinky ():
    ...
    if vxP == 0 and vyP == 0:
        draw_stop (xP, yP)
        canv.create_rectangle (xP - 8, yP - 6, xP - 4, yP - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
        canv.create_rectangle (xP + 4, yP - 6, xP + 8, yP - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
    else:
        if x == xP:
            draw_stop (xP, yP)
            if y > yP:
                canv.create_rectangle (xP - 8, yP - 4, xP - 4, yP, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xP + 4, yP - 4, xP + 8, yP, fill = 'blue', outline = 'blue', tag = 'Ghost')
            else:
                canv.create_rectangle (xP - 8, yP - 10, xP - 4, yP - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xP + 4, yP - 10, xP + 8, yP - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
        elif x > xP:
            draw_right (xP, yP)
            if y >= yP:
...
def draw_Inky ():
    ...
    if vxI == 0 and vyI == 0:
        draw_stop (xI, yI)
        canv.create_rectangle (xI - 8, yI - 6, xI - 4, yI - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
        canv.create_rectangle (xI + 4, yI - 6, xI + 8, yI - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')        
    else:
        if x == xI:
            draw_stop (xI, yI)
            if y > yI:
                canv.create_rectangle (xI - 8, yI - 4, xI - 4, yI, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xI + 4, yI - 4, xI + 8, yI, fill = 'blue', outline = 'blue', tag = 'Ghost')
            else:
                canv.create_rectangle (xI - 8, yI - 10, xI - 4, yI - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xI + 4, yI - 10, xI + 8, yI - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
        elif x > xI:
            draw_right (xI, yI)
            if y >= yI:
...
def draw_Clyde ():
    ...
    if vxC == 0 and vyC == 0:
        draw_stop (xC, yC)
        canv.create_rectangle (xC - 8, yC - 6, xC - 4, yC - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
        canv.create_rectangle (xC + 4, yC - 6, xC + 8, yC - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')        
    else:
        if x == xC:
            draw_stop (xC, yC)
            if y > yC:
                canv.create_rectangle (xC - 8, yC - 4, xC - 4, yC, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xC + 4, yC - 4, xC + 8, yC, fill = 'blue', outline = 'blue', tag = 'Ghost')
            else:
                canv.create_rectangle (xC - 8, yC - 10, xC - 4, yC - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
                canv.create_rectangle (xC + 4, yC - 10, xC + 8, yC - 6, fill = 'blue', outline = 'blue', tag = 'Ghost')
        elif x > xC:
            draw_right (xC, yC)
            if y >= yC:
...
def draw_walls ():
    ...
    for i in range (len (A)):
        ...
        canv.create_text (h + (A [i][0] + A [i][2]) // 2, h + (A [i][1] + A [i][3]) // 2, text = i + 1)
...
def start ():
    ...
    global w, a, f
    global rB, rP, rI, rC
    #x, y = (540 + 2 * h) // 2, (540 + 2 * h) // 2
    x, y = 270 + h, 460 + h
    #xB, yB = (540 + 2 * h) // 4, (540 + 2 * h) // 4
    xB, yB = 270 + h, 240 + h
    #xP, yP = 3 * (540 + 2 * h) // 4, (540 + 2 * h) // 4
    xP, yP = 270 + h, 300 + h
    #xI, yI = (540 + 2 * h) // 4, 3 * (540 + 2 * h) // 4
    xI, yI = 230 + h, 300 + h
    #xC, yC = 3 * (540 + 2 * h) // 4, 3 * (540 + 2 * h) // 4
    xC, yC = 310 + h, 300 + h
    vx, vy = 0, 0
    vxB, vyB = 0, 0
    vxP, vyP = 0, 0
...
v = [1, 0.5]
#A = [[380, 300, 400, 380]]
A = [[40, 40, 100, 80], [140, 40, 220, 80], [260, 0, 280, 80], [320, 40, 400, 80], [440, 40, 500, 80], [40, 120, 100, 140], [140, 120, 160, 260], [200, 120, 340, 140], [380, 120, 400, 260], [440, 120, 500, 140], [260, 140, 280, 200], [160, 180, 220, 200], [320, 180, 380, 200], [140, 300, 160, 380], [380, 300, 400, 380], [200, 360, 340, 380], [260, 380, 280, 440]]
draw_walls ()
draw_lives (l)
start ()
