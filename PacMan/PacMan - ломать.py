...
#from [name_project].settings._django import *
import time
from tkinter import *
root = Tk ()
...
def move_Blinky ():
    global xB, yB, vxB, vyB, x, y, newvxB, newvyB, level
    A = [0] * 31
    ...
    while len (q) > 0:
        ...
        n1 = (j - 1) % 28
        n2 = (j + 1) % 28
        #if m < len (A) and n1 < len (A [i]) and m < len (level) and n1 < len (level [i]):
        if A [i][n1] == -2 and level [i][n1] != 9:
            A [i][n1] = A [i][j] + 1
            q += [(i, n1)]
        #elif level [i][n1] == 9: A [i][n1] = 1000
        #if m < len (A) and n2 < len (A [i]) and m < len (level) and n2 < len (level [i]):
        if A [i][n2] == -2 and level [i][n2] != 9:
            A [i][n2] = A [i][j] + 1
            q += [(i, n2)]
        #elif level [i][n2] == 9: A [i][n2] = 1000
        #if m1 < len (A) and n < len (A [m1]) and m1 < len (level) and n < len (level [m1]):
        if A [m1][j] == -2 and level [m1][j] != 9:
                A [m1][j] = A [i][j] + 1
                q += [(m1, j)]
        #elif level [m1][j] == 9: A [m1][j] = 1000
        #if m2 < len (A) and n < len (A [m2]) and m2 < len (level) and n < len (level [m2]):
        if A [m2][j] == -2 and level [m2][j] != 9:
            A [m2][j] = A [i][j] + 1
            q += [(m2, j)]
        #elif level [m2][j] == 9: A [m2][j] = 1000
...
#def move_Pinky ():
    #global xP, yP, x, y, vxP, vyP
        ...
        #vxP = 0
def move_Pinky ():
    global xP, yP, vxP, vyP, x, y, vx, vy, newvxP, newvyP, level
    A = [0] * 31
    for i in range (31): A [i] = [-2] * 28
    #A [0] = ['+'] * 28
    #A [-1] = ['+'] * 28
    #for i in range (26):
        ...
        #A [i + 1][-1] = '+'
    xPA, yPA = x // 20, y // 20
    if vx < 0: A [yPA % 31][(xPA - 4) % 28] = 0
    elif vx > 0: A [yPA % 31][(xPA + 4) % 28] = 0
    elif vy < 0: A [(yPA - 4) % 31][xPA % 28] = 0
    elif vy > 0: A [(yPA + 4) % 31][xPA % 28] = 0
    q = [(yPA, xPA)]
    while len (q) > 0:
        i, j = q.pop (0)
    ...
    if A [m2][n] < A [m][n] and level [m2][n] != 9:
        ...
        newvxP = 0
    #while A [yPt][yPt] > 0:
        #if A [yPt][xPt - 1] < A [yPt][xPt]:
            #xPt -= 1
            #S += [(yPt, xPt)]
        #elif A [yPt][xPt + 1] < A [yPt][xPt]:
            #xPt += 1
            #S += [(yPt, xPt)]
        #elif A [yPt - 1][xPt] < A [yPt][xPt]:
            #yPt -= 1
            #S += [(yPt, xPt)]
        #elif A [yPt + 1][xPt] < A [yPt][xPt]:
            #yPt += 1
...
#def draw_5 (xG, yG):
    #canv.create_rectangle (xG - 2, yG - 4, xG, yG - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
    #canv.create_rectangle (xG + 4, yG - 4, xG + 12, yG - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
def draw_6 (xG, yG):
...
#def draw_7 (xG, yG):
    #canv.create_rectangle (xG - 6, yG - 4, xG - 4, yG - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')
    #canv.create_rectangle (xG, yG - 4, xG + 2, yG - 2, fill = 'blue', outline = 'blue', tag = 'Ghost')    
...
while l >= 0:
    ...
    if xB % 20 == 10 and yB % 20 == 10:
        if level [(yB // 20 + sign (newvyB)) % 31][(xB // 20 + sign (newvxB)) % 28] != 9:
            ...
            vxB, vyB = 0, 0
    if xP % 20 == 10 and yP % 20 == 10:
            if level [(yP // 20 + sign (newvyP)) % 31][(xP // 20 + sign (newvxP)) % 28] != 9:
                vxP, vyP = newvxP, newvyP
            elif level [(yP // 20 + sign (vyP)) % 31][(xP // 20 + sign (vxP)) % 28] == 9:
                vxP, vyP = 0, 0
    move_pac ()
    xB += vxB
    yB += vyB
